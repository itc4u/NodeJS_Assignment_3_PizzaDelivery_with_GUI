...
const notFound = require("../ApiRequestHandlers/FallbackRequestHandler").notFound;
...
const unifiedServer = async (req, res) => {
    ...
    let chosenHandler = Object.keys(router).includes(trimmedPath) ? router[trimmedPath] : notFound;

    // If the request is sent to "public/..." route, use the public handler instead.
    chosenHandler = trimmedPath.startsWith("public/") ? router["public"] : chosenHandler;
    ...
    const data =  {
        ...
        method,
        headers,
        "payload" : Helper.parseJsonToObject(requestPayload)
    };
    ...
    let payloadString = "";
    if (content_type === "application/json") payloadString = JSON.stringify(typeof(payload) === "object" ? payload : {});
    if (content_type === "text/html") payloadString = typeof(payload) === "string" ? payload : "";
    if (content_type === "image/x-icon") payloadString = payload ? payload : "";
    if (content_type === "text/css") payloadString = payload ? payload : "";
    if (content_type === "image/png") payloadString = payload ? payload : "";
    if (content_type === "image/jpeg") payloadString = payload ? payload : "";
    if (content_type === "application/javascript") payloadString = payload ? payload : "";
    if (content_type === "text/plain") payloadString = payload ? payload : "";
