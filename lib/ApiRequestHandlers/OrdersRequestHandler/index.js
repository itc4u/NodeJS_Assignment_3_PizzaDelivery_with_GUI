    ...
    "post" : async data => {
        ...
        if (email) {
            ...
            try {
                ...
                const transaction = await Helper.sendStripePayment(Number(String(receipt.amount).replace(".", "")), "nzd", `Items Ordered : [ ${Object.entries(receipt.items_ordered).map(value => (`{"${value[0]}":"${value[1]}"} `))}]`, Config["stripe"]["test_token"]);
